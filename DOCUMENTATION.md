...
- **Обновление кода:** `git pull` (или загрузка новой версии), затем `cd backend && npm install --production`, перезапуск PM2: `pm2 restart promptlearn`.
- **Резервная копия:** периодически копируйте файл SQLite (`database.db` или путь из `DB_PATH`) и каталог `backend/uploads/`, если используются вложения к урокам.
...
- **Сервер (VPS/хостинг):** см. раздел 9 — `.env` (`JWT_SECRET`, `CORS_ORIGIN=https://…`), PM2/systemd, nginx перед Node.
