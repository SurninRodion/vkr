...
4. **Проверьте состояние git** (чтобы увидеть отставание от `origin` и лишние локальные правки):
   ...
   Если много изменённых файлов в **`backend/node_modules`** — это нормально после установки зависимостей **на Linux**, если `node_modules` случайно отслеживается git; см. шаг 5.
...
6. **Подтянуть новый код** с `origin`. Предпочтительно быстрая перемотка без лишнего merge-коммита:
   ...
   - для **`backend/node_modules`**: если папка отслеживается git и мешает `pull`, сбросьте её к состоянию репозитория:  
     `git restore backend/node_modules` (или `git checkout -- backend/node_modules`), затем выполните шаг 7 — после **`npm install`** зависимости снова будут собраны **под Linux**.
